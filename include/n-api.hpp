#ifndef SBS_N_API_HPP
#define SBS_N_API_HPP
...
#define SETUP_NAPI_ENVIRONEMENT Napi::Env environement = info.Env();
#define SETUP_NAPI_HANDLE_SCOPE Napi::HandleScope scope(environement);

#define JS_UNDEFINED environement.Undefined()
#define JS_NULL      environement.Null()
#define JS_STR(VAL)  Napi::String::New(environement, VAL)
#define JS_NUM(VAL)  Napi::Number::New(environement, static_cast<double>(VAL))
#define JS_EXT(VAL)  Napi::External<void>::New(environement, static_cast<void*>(VAL))
#define JS_BOOL(VAL) Napi::Boolean::New(environement, static_cast<bool>(VAL))
#define JS_OBJECT    Napi::Object::New(environement)
#define JS_ARRAY     Napi::Array::New(environement)
...
#define JS_THROW(VAL)                                                    \
	Napi::Error::New(environement, VAL).ThrowAsJavaScriptException();


#define REQ_ARGS_COUNT(N)                                                \
	if (info.Length() < (N)) {                                             \
...
#define USE_STR_ARG(I, VAR, DEF)                                         \
	CHECK_OPT_ARG(I, IsString(), "String");                                \
...
#define USE_INT32_ARG(I, VAR, DEF)                                       \
	CHECK_OPT_ARG(I, IsNumber(), "Int32");                                 \
...
#define OPT_UINT32_ARG(I, VAR, DEF)                                    \
	CHECK_OPT_ARG(I, IsNumber(), "Uint32");                              \
	unsigned int VAR = IS_ARG_EMPTY(I)                                   \
		? (DEF)                                                            \
		: info[I].ToNumber().Uint32Value();

#define OPT_UINT32_ARG(I, VAR) USE_UINT32_ARG(I, VAR, 0)

#define REQ_UINT_ARG(I, VAR) REQ_UINT32_ARG(I, VAR)
#define USE_UINT_ARG(I, VAR, DEF) USE_UINT32_ARG(I, VAR, DEF)
#define OPT_UINT_ARG(I, VAR) OPT_UINT32_ARG(I, VAR)
...
#define USE_BOOL_ARG(I, VAR, DEF)                                     \
	CHECK_OPT_ARG(I, IsBoolean(), "Bool");                              \
	bool VAR = IS_ARG_EMPTY(I) ? (DEF) : info[I].ToBoolean().Value();

#define OPT_BOOL_ARG(I, VAR) USE_BOOL_ARG(I, VAR, false)
...
#define USE_OFFS_ARG(I, VAR, DEF)                                     \
	CHECK_OPT_ARG(I, IsNumber(), "Number");                             \
	size_t VAR = IS_ARG_EMPTY(I)                                        \
	? (DEF)                                                             \
	: static_cast<size_t>(info[I].ToNumber().DoubleValue());

#define OPT_OFFS_ARG(I, VAR) USE_OFFS_ARG(I, VAR, 0)
...
#define OPT_DOUBLE_ARG(I, VAR) USE_DOUBLE_ARG(I, VAR, 0.0)
...
#define OPT_FLOAT_ARG(I, VAR) USE_FLOAT_ARG(I, VAR, 0.f)
...
#define USE_EXT_ARG(I, VAR, DEF)                                      \
	CHECK_OPT_ARG(I, IsExternal(), "Pointer");                          \
	void *VAR = IS_ARG_EMPTY(I) ? (DEF) : info[I].As< Napi::External<void> >().Data();

#define OPT_EXT_ARG(I, VAR) USE_EXT_ARG(I, VAR, nullptr)
...
#define USE_OBJ_ARG(I, VAR, DEF)                                      \
	CHECK_OPT_ARG(I, IsObject(), "Object");                             \
	Napi::Object VAR = IS_ARG_EMPTY(I) ? (DEF) : info[I].As<Napi::Object>();

#define OPT_OBJ_ARG(I, VAR) USE_OBJ_ARG(I, VAR, Napi::Object::New(environement))
...
#define USE_ARRAY_ARG(I, VAR, DEF)                                    \
	CHECK_OPT_ARG(I, IsArray(), "Array");                               \
	Napi::Array VAR = IS_ARG_EMPTY(I) ? (DEF) : info[I].As<Napi::Array>();

#define OPT_ARRAY_ARG(I, VAR) USE_ARRAY_ARG(I, VAR, Napi::Array::New(environement))
...
#define GET_AND_THROW_LAST_ERROR()                                    \
	do {                                                                \
		...
		napi_get_last_error_info((environement), &error_info);                     \
		bool is_pending;                                                  \
		napi_is_exception_pending((environement), &is_pending);                    \
...
#define JS_RUN(code, VAR)                                             \
	...
	NAPI_CALL(                                                          \
		napi_run_script(environement, napi_value(JS_STR(code)), &__RESULT_ ## VAR) \
	);                                                                  \
	Napi::Value VAR(environement, __RESULT_ ## VAR);


template<typename Type = uint8_t>
inline Type* GetArrayData(
	Napi::Env environement,
	Napi::Object obj,
	int *num = nullptr
) {
...
inline void *GetData(Napi::Env environement, Napi::Object obj) {
	void *out = nullptr;
	if (obj.IsTypedArray() || obj.IsArrayBuffer()) {
		out = GetArrayData<uint8_t>(environement, obj);
	} else if (obj.IsBuffer()) {
		out = GetBufferData<uint8_t>(environement, obj);
	} else if (obj.Has("data")) {
		Napi::Object data = obj.Get("data").As<Napi::Object>();
		if (data.IsTypedArray() || data.IsArrayBuffer()) {
			out = GetArrayData<uint8_t>(environement, data);
		} else if (data.IsBuffer()) {
			out = GetBufferData<uint8_t>(environement, data);
		}
	}
...
inline Napi::Value ConsoleLog(Napi::Env environement, const std::string &message) {
	Napi::Value arg = JS_STR(message);
	ConsoleLog(environement, 1, &arg);
	RET_UNDEFINED;
}
...
inline void EventEmit(
	...
	Napi::Env environement = that.Env();
	
	Napi::String event = JS_STR(name);
	Napi::Function emit = that.Get("emit").As<Napi::Function>();
	
	std::vector<napi_value> args;
	args.push_back(napi_value(event));
	for (int i = 0; i < argc; i++) {
		args.push_back(napi_value(argv[i]));
	}
...
inline void EventEmitAsync(
	...
	emit.MakeCallback(that, args, context);
...
private:                                                               \
	...
	inline static Napi::Function wrap(Napi::Env environement) {                   \
		napi_value __initResult;                                           \
		napi_create_function(                                              \
			environement, #NAME, 0, _createEs5, nullptr, &__initResult                \
		);                                                                 \
		Napi::Function ctor = Napi::Function(environement, __initResult);           \
...
#define IMPLEMENT_ES5_CLASS(CLASS)                                     \
	...
	napi_value CLASS::_createEs5(napi_env environement, napi_callback_info i) {   \
		Napi::CallbackInfo info(environement, i);                                   \
		CLASS *instance = new CLASS(info);                                 \
		Napi::Object wrapObj = Napi::Object::New(environement);                     \
		info.This().As<Napi::Object>().Set(_nameEs5, wrapObj);             \
		napi_wrap(environement, wrapObj, instance, _finalizeEs5, nullptr, nullptr); \
